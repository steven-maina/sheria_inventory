...
$(document).on('submit', '#add-item-form', function(event) {
	...
	var form_data = new Array(
								...
								$('#conID'),
								$('input[id=os]'),
								$('input[id=ramCapacity]'),
								$('input[id=romCapacity]'),
								$('input[id=roomNo]'),
								$('input[id=moreDescription]')								
	...
	for(var i = 0; i < form_data.length; i++){
		...
		if(roomNo == ""){
			$("#error").fadeIn().text("Room No: is field required");
			$("input#roomNo").focus();
			return false;
		}
	$.ajax({
		...
		success: function(event){
			swal("success...", "functions starts!!", "success");
        alert("upto here");
			if(event.valid == valid){
				...
				action = event.action;
				$("#show_message").fadeIn().text("data saved successfully");
				show_all_item();
			}
		},
		error: function(){
...
//	}
	//swal("could not save!", "error with your form data", "error");
	//end validate
...
function show_all_item()
{
	$.ajax({
		...
		error: function(){
			swal("could not show all!", "error with your form data", "error");
...
var off_id;
function fill_eqCategory_form(id){
	off_id = id;
